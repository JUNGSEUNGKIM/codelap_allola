...
const WEB_SERVER_HOME = 'C:\\BEJ\\utill\\nginx-1.24.0\\nginx-1.24.0\\html';
...
router.get('/', async (req, res) => {
    ...
    res.render('ej_create', {
        userId: req.session.loggedInUserCode,
        username: req.session.loggedInUserId,
...
router.post('/',  upload.array('files', 5),async (req, res) => {
    ...
    const userCode = req.session.loggedInUserCode; // 현재 로그인한 사용자의 ID
    let conn;
    try {
