...
router.get('/:boarderCode', async (req, res) => {
    ...
    try {
        ...
        const result = await conn.execute(
            `SELECT * FROM customshop WHERE boarder_code = :boarderCode`,
            [boarderCode],
...
router.post('/:boarderCode', async (req, res) => {
    ...
    try {
        ...
        // 게시글 수정
        await conn.execute(
            `UPDATE customshop SET title = :title, content = :content, price = :price WHERE boarder_code = :boarderCode`,
            [title, content, price, boarderCode]
        );
        ...
        // 수정 후 상세 페이지로 리다이렉트
        res.redirect(`/customshop_detailPost/${boarderCode}?user_id=${req.session.userId}&username=${req.session.username}&user_realname=${req.session.userRealName}`);
    } catch (err) {
