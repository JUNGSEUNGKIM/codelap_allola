...
router.get('/:boarderCode', async (req, res) => {
    ...
    try {
        ...
        // 게시글에 달린 댓글과 답글 삭제
        await conn.execute(
            `DELETE FROM customshop_comments WHERE boarder_code = :boarderCode OR parent_comment_id IN (SELECT id FROM customshop_comments WHERE boarder_code = :boarderCode)`,
        ...
        // 게시글 삭제
        await conn.execute(
            `DELETE FROM customshop WHERE boarder_code = :boarderCode`,
            [boarderCode]
        );
        ...
        res.redirect(`/customshop_boardMain?id=${userId}&username=${userName}&name=${userRealName}`);
    } catch (err) {
